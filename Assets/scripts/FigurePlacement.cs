...
public class FigurePlacement : MonoBehaviour
{
    ...
    public Transform White_Parent;
    public Transform Black_Parent;
    ...
    void Start()
    {
        ...
        for (int width = 0; width < 8; width++)//white spawn
        {
            for (int lenght = 0; lenght > -2; lenght--)
            {
                ...
                var piece = Instantiate(piecePrefab, position, Quaternion.Euler(pieceRotation));
                piece.name = piece.name + " " + width + lenght;
                piece.transform.SetParent(White_Parent);
                piece.transform.localScale *= PieceScale;
                piece.AddComponent<BoxCollider>();  // Добавление BoxCollider
                var clickHandler = piece.AddComponent<PieceClickHandler>();  // Добавление обработчика кликов
                clickHandler.BluePad = BluePad;
                clickHandler.YellowPad = YellowPad;
                clickHandler.RedPad = RedPad;
            }
        ...
        for (int width = 0; width < 8; width++)//black spawn
        {
            for (int lenght = -6; lenght > -8; lenght--)
            {
                ...
                var piece = Instantiate(piecePrefab, position, Quaternion.Euler(pieceRotation));
                piece.name = piece.name + " " + width + lenght;
                piece.transform.SetParent(Black_Parent);
                piece.transform.localScale *= PieceScale;
                piece.AddComponent<BoxCollider>();  // Добавление BoxCollider
                var clickHandler = piece.AddComponent<PieceClickHandler>();  // Добавление обработчика кликов
                clickHandler.BluePad = BluePad;
